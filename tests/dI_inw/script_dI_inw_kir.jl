...
include("membrane_inw.jl")
include("fixedpoints_inw.jl")
...
### SCRIPT INPUTS ###
i_gKir_inw = parse(Int64, ARGS[1]) #1:length(gKir_KirCaLs_inw)
...
### INPUTS ###
    ...
    jld_name = "data/jld/dI_KirCaLs_inw"

### TASK ###
    batch_inw = i_gKir_inw:(minimum([i_gKir_inw+n_batch_inw-1,length(gKir_KirCaLs_inw)]))
    for i_batch in batch_inw
        jld_name_i = string(jld_name,"_i_gKir_$(i_batch).jld")
        p_var_i_batch = [I,gNa,gKDR,gKir_KirCaLs_inw[i_batch],0.,pCaLf,pCaLs_KirCaLs_inw,gleak]
        find_dI_ = find_dI([p_var_i_batch,p_fixed],ic,[],[],[],[gKir_KirCaLs_inw[i_batch]],[],[-4])
