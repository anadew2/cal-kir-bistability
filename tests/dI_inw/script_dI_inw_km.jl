...
### SCRIPT INPUTS ###
i_gKM_inw = parse(Int64, ARGS[1]) #1:length(gKM_KMCaLs_inw)
...
### INPUTS ###
    ...
    jld_name = "data/jld/dI_KMCaLs_inw"

### TASK ###
    batch_inw = i_gKM_inw:(minimum([i_gKM_inw+n_batch_inw-1,length(gNa_var_mat)]))
    for i_batch in batch_inw
        jld_name_i = string(jld_name,"_i_gKM_$(i_batch).jld")
        p_var_i_batch = [I,gNa,gKDR,0.,gKM_KMCaLs_inw[i_batch],pCaLf,pCaLs_KMCaLs_inw,gleak]
        find_dI_ = find_dI([p_var_i_batch,p_fixed],ic,[],[],[],[],[gKM_KMCaLs_inw[i_batch]],[-4])
