...
## Varying parameters
	I1bist = I1_KirCaLs[32,end]
	I2bist = I2_KirCaLs[32,end]
	I0 = (I1_KirCaLs[32,end]+I2_KirCaLs[32,end])/2

	I(t) = I0  +  2. *pulse(t, 15000, 15000+1000)  # [µA/cm^2]gNa = 30 #mS/cm²
	gKDR = 4 #mS/cm²
	gleak = 0.03268 #mS/cm²
	gKir = gKir_range_KirCaLs[end]           # [mS/cm²]
	gKM = 0.           # [mS/cm²]
	pCaLf = 0. /1000    # [cm/s]
	pCaLs = pCaLs_range_KirCaLs[32]   # [cm/s]
...
## Initial conditions for spiking state
	...
	display(plot(sol.t,sol[1,:],xlabel="t",ylabel="V",fontfamiliy="Computer Modern"))
	display(plot(sol.t[Int(round(length(sol.t)*3/4)):end],sol[1,Int(round(length(sol.t)*3/4)):end],xlabel="t",ylabel="V",fontfamiliy="Computer Modern"))
	sol_cb = solve(prob,callback=cb,save_everystep=false,save_start=false,save_end=false,Rodas5P(),dtmax=1,reltol=1e-7,abstol=1e-7)
	Plots.scatter!(sol_cb.t[2:end],sol_cb[1,2:end],xlabel="t",ylabel="V",fontfamiliy="Computer Modern")
