...
function membrane_grad_0(V,Ca,p)
	...
	## Varying parameters
	I,gNa,gKDR,gKir,gKM,gCaLf,gCaLs,gleak = p[1]
    ...
    ICaLf_ = ICaLf(gCaLf,mCaLf_inf,hCaLf_inf,V,Ca,Ca_o)
    ICaLs_ = ICaLs(gCaLs,mCaLs_inf,hCaLs_inf,V,Ca,Ca_o)
...
function nlsolve_membrane_grad_0_I_!(F,x,p_)
    ## Varying parameters
	I,gNa,gKDR,gKir,gKM,gCaLf,gCaLs,gleak = p_[1]
	...
	p_var=[x[1],gNa,gKDR,gKir,gKM,gCaLf,gCaLs,gleak]
    p=[p_var,p_fixed]
...
function J_dICaLf_dmCaLf(V,Ca,Ca_o)
	...
	dICaLf_dmCaLf = 2*mCaLf_inf*hCaLf_inf*(V-eCa)
...
function J_dICaLf_dhCaLf(V,Ca,Ca_o)
	...
	dICaLf_dhCaLf = mCaLf_inf^2*(V-eCa)
...
function compute_dVdot_dV(V,Ca,p,step)
    ...
    dICaLf_dV  = gCaLf*mCaLf_inf^2*hCaLf_inf

    mCaLs_inf=mCaLs(V)
    hCaLs_inf=hCaLs(V)
    #dICaLs_dV  = pCaLs*mCaLs_inf*hCaLs_inf*compute_dghk_dV(V,Ca,Ca_o,step) *1000 
    dICaLs_dV  = gCaLs*mCaLs_inf*hCaLs_inf
...
function J_dmCaLfdot_dV(V,step)
	tau_mCaLf_=tau_mCaLf(V)
	dm_inf_dV = derivative(mCaLf,step,V)
	J = dm_inf_dV/tau_mCaLf_
	return J
end
function J_dhCaLfdot_dV(V,step)
	tau_hCaLf_=tau_hCaLf(V)
	dh_inf_dV = derivative(hCaLf,step,V)
...
function J_matrix(V,Ca,p)
	...
	J_mat[1,5] = -gCaLf * J_dICaLf_dmCaLf(V,Ca,Ca_o)
	J_mat[1,6] = -gCaLf * J_dICaLf_dhCaLf(V,Ca,Ca_o)
    J_mat[1,7] = -gCaLs * J_dICaLs_dmCaLs(V,Ca,Ca_o)
	J_mat[1,8] = -gCaLs * J_dICaLs_dhCaLs(V,Ca,Ca_o)
	...
	J_mat[5,5] = -1/tau_mCaLf(V)
	J_mat[6,6] = -1/tau_hCaLf(V)
...
function stability_FP(p,I_l_lf,I_h_lf,step_Illf_to_Ihlf)
	...
	## Varying parameters
	I,gNa,gKDR,gKir,gKM,gCaLf,gCaLs,gleak = p_var
    ...
    for i in range(I_l_lf,stop=I_h_lf,step=step_Illf_to_Ihlf)
        ...
        p_var_i = [i,gNa,gKDR,gKir,gKM,gCaLf,gCaLs,gleak]
