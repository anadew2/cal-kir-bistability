...
function bif_asc_w_pulse_if_bist(ode_fun,I_list,p,ic_h,I1,tmin)
    ## Old name: bif_diag_pulse_aroundbist(ode_fun,FP_fun,ode_fun_pulse,I_list,p,ic_h,I1,tmin)
...
function find_bif(p,ic,I_list_0f,I_list_lf,I_list_hf,I1)
    ## Old name: find_full_bif_dI_only(p,ic,I_list_0f,I_list_lf,I_list_hf,I1)
...
function find_dI(p,ic,pCaLf_,pCaLs_,gKDR_,gKir_,gKM_,I_X)
    ## Old name:  find_dI_only_eig_type(p,ic,gCaL_,gKDR_,gKir_,gKM_,I_X,est_type)
    ...
    for i in eachindex(p_to_test)
        ...
        popfirst!(big_Istep)
        I1_est,bif_0f,bif_lf,bif_hf = find_bif([p_to_test[i],p_fixed],ic,[],I_list_lf_big,[],ceil(I_SN)+2)
...
### SCRIPT INPUTS ###
    i_pCaLs_range_KirKMCaLs= parse(Int64, ARGS[1])           #1:length(pCaLs_range_KirKMCaLs)
    i_gKir_range_KirKMCaLs= parse(Int64, ARGS[2])           #1:length(gKir_range_KirKMCaLs)
    i_batch_gKM_range_KirKMCaLs = parse(Int64, ARGS[3])      #1:n_batch_gKM_range_KirKMCaLs:length(gKM_range_KirKMCaLs)

### INPUTS ###
    n_batch_gKM_range_KirKMCaLs = 1

    V_ic = -90.     # [mV]
    Ca_ic = 5.0e-5  # [mM]
    ic = [V_ic,mNa(V_ic),hNa(V_ic),mKDR(V_ic),mCaLf(V_ic),hCaLf(V_ic),mCaLs(V_ic),hCaLs(V_ic),mKM(V_ic),mKM(V_ic),Ca_ic]

    jld_name = "data/jld/dI_KirKMCaLs_"

### TASK ###
    batch_gKM_range_KirKMCaLs = i_batch_gKM_range_KirKMCaLs:(minimum([i_batch_gKM_range_KirKMCaLs+n_batch_gKM_range_KirKMCaLs-1,length(gKM_range_KirKMCaLs)]))
    for i_gKM_range_KirKMCaLs in batch_gKM_range_KirKMCaLs
        jld_name_i = string(jld_name,"i_pCaLs_$(i_pCaLs_range_KirKMCaLs)_i_Kir_$(i_gKir_range_KirKMCaLs)_i_gKM_$(i_gKM_range_KirKMCaLs).jld")
        p_var_i_batch = [I,gNa,gKDR,gKir_range_KirKMCaLs[i_gKir_range_KirKMCaLs],gKM_range_KirKMCaLs[i_gKM_range_KirKMCaLs],pCaLf,pCaLs_range_KirKMCaLs[i_pCaLs_range_KirKMCaLs],gleak]
        find_dI_ = find_dI([p_var_i_batch,p_fixed],ic,[],[],[],[],[gKM_range_KirKMCaLs[i_gKM_range_KirKMCaLs]],[IX_KirKMCaLs[i_pCaLs_range_KirKMCaLs][i_gKM_range_KirKMCaLs]])
