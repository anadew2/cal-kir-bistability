...
function bif_desc_w_pulse(ode_fun,I_list,p,ic_h,I1,tmin)
    ...
    for i=1:length(I_list)
        ...
        while freq_test_tol > f_tol && cnt <lim_cnt
            ...
            sol = solve(prob,callback=cb,save_everystep=false,save_start=false,save_end=false,abstol=1e-7,reltol=1e-7,Rodas5P())
...
### SCRIPT INPUTS ###
    i_pCaLs_range_KMCaLs= parse(Int64, ARGS[1])           #1:length(pCaLs_range_KMCaLs)
    i_batch_gKM_range_KMCaLs = parse(Int64, ARGS[2])      #1:n_batch_gKM_range_KMCaLs:length(gKM_range_KMCaLs)

### INPUTS ###
    n_batch_gKM_range_KMCaLs = 1

    V_ic = -90.     # [mV]
    Ca_ic = 5.0e-5  # [mM]
    ic = [V_ic,mNa(V_ic),hNa(V_ic),mKDR(V_ic),mCaLf(V_ic),hCaLf(V_ic),mCaLs(V_ic),hCaLs(V_ic),mKM(V_ic),mKM(V_ic),Ca_ic]

    jld_name = "data/jld/dI_KMCaLs_other_Kv72_"

### TASK ###
    batch_gKM_range_KMCaLs = i_batch_gKM_range_KMCaLs:(minimum([i_batch_gKM_range_KMCaLs+n_batch_gKM_range_KMCaLs-1,length(gKM_range_KMCaLs)]))
    for i_gKM_range_KMCaLs in batch_gKM_range_KMCaLs
        jld_name_i = string(jld_name,"i_pCaLs_$(i_pCaLs_range_KMCaLs)_i_gKM_$(i_gKM_range_KMCaLs).jld")
        p_var_i_batch = [I,gNa,gKDR,gKir,gKM_range_KMCaLs[i_gKM_range_KMCaLs],pCaLf,pCaLs_range_KMCaLs[i_pCaLs_range_KMCaLs],gleak]
        find_dI_ = find_dI([p_var_i_batch,p_fixed],ic,[],[],[],[],[gKM_range_KMCaLs[i_gKM_range_KMCaLs]],[IX_KMCaLs[i_pCaLs_range_KMCaLs][i_gKM_range_KMCaLs]])
