...
pulse(t,ti,tf)=heaviside(t-ti)-heaviside(t-tf)
    ...
    eCa = 120 # [mV]
    eleak = -60.1 # [mV]
    Ca_o = NaN # Extracellular Ca concentration [mM]
    ...
    ## Varying parameters
        #p = [-0.3,30,4,0,0,0,0.1,0.03268] --> bistable for V_ic = -50 or -100
        #p = [-0.3,30,4,0,0,0,0.01/1000,0.03268] --> bistable for V_ic = -50 or -100 (after modfif in membrane!() )
    I(t) = -0.3 + 2.1*pulse(t, 5000, 5000+1000)  # [µA/cm^2]
    gNa = 30 #mS/cm²
    gKDR = 4 #mS/cm²
    gleak = 0.03268 #mS/cm²
    gKir = 0.           # mS/cm²
...
plot(sol.t,ICaLs.(gCaLs,sol[7,:],sol[8,:],sol[1,:],sol[11,:],Ca_o))
