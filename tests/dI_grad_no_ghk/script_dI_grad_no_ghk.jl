...
### SCRIPT INPUTS ###
    i_gCaLs_range_KirCaLs= parse(Int64, ARGS[1])           #1:length(gCaLs_range_KirCaLs)
    i_batch_gKir_range_KirCaLs = parse(Int64, ARGS[2])      #1:n_batch_gKir_range_KirCaLs:length(gKir_range_KirCaLs)

### INPUTS ###
    n_batch_gKir_range_KirCaLs = 6

    V_ic = -90.     # [mV]
    Ca_ic = 5.0e-5  # [mM]
    ic = [V_ic,mNa(V_ic),hNa(V_ic),mKDR(V_ic),mCaLf(V_ic),hCaLf(V_ic),mCaLs(V_ic),hCaLs(V_ic),mKir(V_ic),mKM(V_ic),Ca_ic]

    jld_name = "data/jld/dI_KirCaLs_larger_range_no_ghk_"

### TASK ###
    batch_gKir_range_KirCaLs = i_batch_gKir_range_KirCaLs:(minimum([i_batch_gKir_range_KirCaLs+n_batch_gKir_range_KirCaLs-1,length(gKir_range_KirCaLs)]))
    for i_gKir_range_KirCaLs in batch_gKir_range_KirCaLs
        jld_name_i = string(jld_name,"i_gCaLs_$(i_gCaLs_range_KirCaLs)_i_gKir_$(i_gKir_range_KirCaLs).jld")
        println("------------ Simulation for i_gCaLs = $(i_gCaLs_range_KirCaLs) & i_gKir = $(i_gKir_range_KirCaLs) ------------")
        p_var_i_batch = [I,gNa,gKDR,gKir_range_KirCaLs[i_gKir_range_KirCaLs],gKM,gCaLf,gCaLs_range_KirCaLs[i_gCaLs_range_KirCaLs],gleak]
        find_dI_ = find_dI([p_var_i_batch,p_fixed],ic,[],[],[],[gKir_range_KirCaLs[i_gKir_range_KirCaLs]],[],[IX_KirCaLs[i_gCaLs_range_KirCaLs][i_gKir_range_KirCaLs]])
