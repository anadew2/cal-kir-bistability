...
### SCRIPT INPUTS ###
i_gKM_outw = parse(Int64, ARGS[1]) #1:length(gKM_KMCaLs_outw)
...
### INPUTS ###
    ...
    jld_name = "data/jld/dI_KMCaLs_outw"

### TASK ###
    batch_outw = i_gKM_outw:(minimum([i_gKM_outw+n_batch_outw-1,length(gKM_KMCaLs_outw)]))
    for i_batch in batch_outw
        jld_name_i = string(jld_name,"_i_gKM_$(i_batch).jld")
        p_var_i_batch = [I,gNa,gKDR,0.,gKM_KMCaLs_outw[i_batch],pCaLf,pCaLs_KMCaLs_outw,gleak]
        find_dI_ = find_dI([p_var_i_batch,p_fixed],ic,[],[],[],[],[gKM_KMCaLs_outw[i_batch]],[-4])
