...
### INPUTS ###
    ...
    jld_name = "data/jld/dI_KirCaLs_outw"

### TASK ###
    batch_outw = i_gKir_outw:(minimum([i_gKir_outw+n_batch_outw-1,length(gKir_KirCaLs_outw)]))
    for i_batch in batch_outw
        jld_name_i = string(jld_name,"_i_gKir_$(i_batch).jld")
        p_var_i_batch = [I,gNa,gKDR,gKir_KirCaLs_outw[i_batch],0.,pCaLf,pCaLs_KirCaLs_outw,gleak]
        find_dI_ = find_dI([p_var_i_batch,p_fixed],ic,[],[],[],[gKir_KirCaLs_outw[i_batch]],[],[-4])
