...
function plt_V_It_noisy_inc_sigma(t_,V,t_It_,It,t_sigma_,sigma,I1bist,I2bist,mean_It,fill_c,yticks_,sigma_ticks_)
    ...
    #axis attributes 
    plot!(xlims=(-maximum(t) *0.03,maximum(t) *1.03),xticks=[minimum(t),maximum(t)],xlabel=L"t \ \ "  *L"[" *latexify(u"ms") *L"]",xlabelfontsize=fontsize_)
    ...
    plot!(xlims=(-maximum(t) *0.03,maximum(t) *1.03),xticks=[minimum(t),maximum(t)])
...
annotate!(0.25,0.18,L"\mathrm{Kir}",annotationfontsize=45,annotationhalign=:center,annotationfontfamily="Computer Modern",annotationcolor=:maroon3)
...
plot!(std_vec_Kir,prop_switch_std_Kir,lc=:maroon3,lw=2,marker=:circle,mc=:maroon3,ms=5,markerstrokewidth=0,label="From resting")
plot!(std_vec_Kir_cyc,prop_switch_std_Kir_cyc,lc=:maroon3,lw=2,marker=:circle,mc=:maroon3,ms=5,markerstrokewidth=0,label="From spiking",ls=:dash)
...
plt_f6_v8 = Plots.plot(empty_plt,plt_noisy_inc_,empty_plt,layout=@layout[a{0.003h} ; b ; c{0.001h}],fontfamily="Computer Modern",foreground_color=:gray25)

#Plots.savefig(plt_f6_v8,"tests/figures/pdf/fig-4.pdf")
