...
function plt_dI_grad(dI_grad_mat,gx,gy,clims_dI,xname,yname)
    palette = cgrad(:seaborn_icefire_gradient ,rev=false)
    fontsize_ =25
...
function plt_Veq_grad(Veq_grad_mat,gx,gy,clims_V,xname,yname)
    ...
    plt_annotation = Plots.plot()
    annotate!(0.5,0.52, latexstring("     ",L"\overline{V}(I_1)"),annotationfontsize=fontsize_)
    plot!(yticks =:none)
    plot!(xticks =:none)
    plot!(grid=false,legend=:none)
    plot!(foreground_color_border=:white,showaxis=false,foreground_color_axis=:white)
...
plt_dI_KMCaLs = plt_dI_grad(dI_KMCaLs,gKM_range_KMCaLs,pCaLs_range_KMCaLs,(0,3.),L"\bar{g}_{\mathrm{KM}}",L"\bar{p}_{\mathrm{CaL}}")
plt_V_I1_KMCaLs = plt_Veq_grad(V_I1_KMCaLs,gKM_range_KMCaLs,pCaLs_range_KMCaLs,(-150,-55),L"\bar{g}_{\mathrm{KM}}",L"\bar{p}_{\mathrm{CaL}}")

plt_dI_KirCaLs = plt_dI_grad(dI_KirCaLs,gKir_range_KirCaLs,pCaLs_range_KirCaLs,(0,3.),L"\bar{g}_{\mathrm{Kir}}",L"\bar{p}_{\mathrm{CaL}}")
plt_V_I1_KirCaLs = plt_Veq_grad(V_I1_KirCaLs,gKir_range_KirCaLs,pCaLs_range_KirCaLs,(-150,-55),L"\bar{g}_{\mathrm{Kir}}",L"\bar{p}_{\mathrm{CaL}}")

plt_kirgrad = Plots.plot(title_kir_km_2_A,plt_dI_KirCaLs,title_kir_km_2_B,plt_V_I1_KirCaLs,layout=@layout[a{0.001h} ; b ; c{0.001h} ; d],size=(1100,1700))
plt_kmgrad = Plots.plot(title_kir_km_2_C,plt_dI_KMCaLs,title_kir_km_2_D,plt_V_I1_KMCaLs,layout=@layout[a{0.001h} ; b ; c{0.001h} ; d],size=(1100,1700))
plt_kir_km_2_ = Plots.plot(empty_plt,plt_kmgrad,empty_plt,plt_kirgrad,empty_plt,layout=@layout[a{0.001w} b c{0.07w} d e{0.001w}],size=(2300,1200),fontfamily="Computer Modern")
plt_kir_km_2 = Plots.plot(empty_plt,plt_kir_km_2_,empty_plt,layout=@layout[a{0.001h} ; b ; c{0.001h}],fontfamily="Computer Modern")

#Plots.savefig(plt_kir_km_2,"ChannelUpdate/cluster/figures/pdf/fig-3.pdf")
