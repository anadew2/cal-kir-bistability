...
ylims_ = (-0.1,4.1)
xlabel_=L"p_\mathrm{CaL}"
plt_kir_pCaLf_xp = Plots.plot(fontfamily="Computer Modern",xlabel=xlabel_,ylabel=L"\Delta I ",ylims=ylims_,legend=:outertopright,palette=pal_kir)

ylims_ = (-0.1,4.1)
plt_km_pCaLf_xp = Plots.plot(fontfamily="Computer Modern",xlabel=xlabel_,ylabel=L"\Delta I ",ylims=ylims_,legend=:outertopright,palette=pal_km)
...
include("../dI_grad/local_postpro_output.jl")
    ...
    for i in eachindex(gKir_range_KirCaLs[1:3:end])
        push!(label_kir_pCaLf_0_,L"p_\mathrm{CaLf}=0, \ g_{\mathrm{Kir}}=%$(round(gKir_range_KirCaLs[1:3:end][i]*10)/10) ")
    end
    ...
    for i in eachindex(gKM_range_KMCaLs[1:3:end])
        push!(label_km_pCaLf_0,L"p_\mathrm{CaLf}=0, \ g_{\mathrm{KM}}=%$(round(gKM_range_KMCaLs[1:3:end][i]*10)/10) ")
    end
...
include("../dI_grad/local_postpro_output_pCaLf_e-4.jl")
    ...
    for i in eachindex(gKir_range_KirCaLs[1:3:end])
        push!(label_kir_pCaLf_1e_4_,L"p_\mathrm{CaLf}=1.5e-4, \ g_{\mathrm{Kir}}=%$(round(gKir_range_KirCaLs[1:3:end][i]*10)/10) ")
    end
    ...
    for i in eachindex(gKM_range_KMCaLs__pCaLf_e_4)
        push!(label_km_pCaLf_1e_4,L"p_\mathrm{CaLf}=1.5e-4, \ g_{\mathrm{KM}}=%$(round(gKM_range_KMCaLs__pCaLf_e_4[i]*10)/10) ")
    end
...
#Plots.savefig(plt_full,"ChannelUpdate/cluster/figures/pdf/fig-A2-pCaLf_increased.pdf")
